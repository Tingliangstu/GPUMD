...
same MD run. Each command must use a distinct filename, so every output file
contains one group sampled at the same simulation times::

  dump_netcdf 0 0 10 1 group_0.nc compression deflate 1
  dump_netcdf 0 1 10 1 group_1.nc compression deflate 1
...
* This keyword can be invoked multiple times within one run to write different
  groups to different files. Each invocation must use a distinct filename.
