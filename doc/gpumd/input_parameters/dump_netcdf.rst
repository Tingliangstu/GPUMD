...
The :attr:`grouping_method` and :attr:`group_id` parameters select the atoms to output, following the same rules as :ref:`dump_xyz <kw_dump_xyz>`. A negative :attr:`grouping_method` selects the whole system.
...
* Repeating the keyword with the same filename for different runs in the same simulation appends to that file.
